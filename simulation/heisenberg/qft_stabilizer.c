...
void cofactor (SINGLE_FRAME* frame, unsigned int num_qubit, unsigned int total_state, unsigned int qubit_index)
{
	...
	//printf("DEBUG2!!!\n");
	COMPLEX_NUM *amp_ref;
	amp_ref = (COMPLEX_NUM*) malloc ((total_state) * sizeof(COMPLEX_NUM)); if(amp_ref==NULL){printf("--Error!: Malloc complex amplitudes failed.--\n");return;}
	//For oversize phase vector for randomized outcome case
	unsigned int swap_index1, swap_index2, found, unsigned_temp;	
	COMPLEX_NUM *amp_ref2;
	amp_ref2 = (COMPLEX_NUM*) malloc ((total_state) * sizeof(COMPLEX_NUM)); if(amp_ref2==NULL){printf("--Error!: Malloc complex amplitudes failed.--\n");return;}
	COMPLEX_NUM *amplitude2;
	amplitude2 = (COMPLEX_NUM*) malloc ((total_state) * sizeof(COMPLEX_NUM)); if(amplitude2==NULL){printf("--Error!: Malloc complex amplitudes failed.--\n");return;}
	COMPLEX_NUM amp_gen2, factor2, complex_temp;
	...
	if (anticommute == 1)
	{
		//printf("COFACTOR: Randomized Measurement Outcome\n");
		//Case 1: Original phase vector sets do not exceed the total_state limit
		if (frame->pair_count < total_state)
		{			
			//Step Factor 1: Obtain non-zero amplitude each for 0 and 1 at position x respectively for each phase vector pair 
			//create and verify the function
			for (i=0;i<frame->pair_count;i++)
			{extract_specific_qubit (&*frame, i, num_qubit, &basis_index[i], &amp_ref[i], &basis_index[i+frame->pair_count], &amp_ref[i+frame->pair_count], qubit_index);}
			...
			//Step 3: For each vector pair, double the size
			current_pair_count = frame->pair_count;
			for (i=0;i<current_pair_count;i++)
			{
				//+ and - for the anticommute row position
				for (j=0;j<num_qubit;j++)	
					{frame->phase[frame->pair_count][j] = frame->phase[i][j];}
				frame->phase[i][anticommute_row] = 0;frame->phase[frame->pair_count][anticommute_row] = 1;
				//Copy the amplitude vector over
				frame->amplitude[frame->pair_count].r = frame->amplitude[i].r; frame->amplitude[frame->pair_count].i = frame->amplitude[i].i;
				frame->pair_count++;
			}		
			//Perform canonical reduction to ensure correct basis amplitude extraction
			canonical_reduction (&*frame, num_qubit, total_state);

			//Extract corresponding amplitudes after cofactor to compute the factor to be included in amplitude vector
			for (i=0;i<frame->pair_count;i++)
			{
				extract_specific (&*frame, i, num_qubit, basis_index[i], &amp_gen);
				complex_div(amp_ref[i], amp_gen, &factor);
				complex_mul(factor, frame->amplitude[i], &frame->amplitude[i]);
			}
		}
		//Case 2: Original phase vector sets reach the total_state limit
		else
		{
//			printf("SPECIAL: Ordinary cofactor approach results in oversize phase vector list!!!\n");
			//Step Factor 1: Obtain non-zero amplitude each for 0 and 1 at position x respectively for each phase vector pair 
			//List 1: + for qubit_index position
			//List 2: - for qubit_index position 
			//printf("anticommute_row: %u\n", anticommute_row);
//			printf("\nInitial Extraction:\n");
			for (i=0;i<total_state;i++)
			{
				//Retain the sequence of phase vector in the original list
				if (frame->phase[i][anticommute_row] == 0)
					extract_specific_qubit (&*frame, i, num_qubit, &basis_index[i], &amp_ref[i], &basis_index2[i], &amp_ref2[i], qubit_index);
				else
					extract_specific_qubit (&*frame, i, num_qubit, &basis_index2[i], &amp_ref2[i], &basis_index[i], &amp_ref[i], qubit_index);
				swap_flag[i] = 0;
//				printf("basis_index[%u]\t: %u: %f + %fi\t\tbasis_index2[%u]\t: %u: %f + %fi\n", i, basis_index[i], amp_ref[i].r, amp_ref[i].i,  i, basis_index2[i], amp_ref2[i].r, amp_ref2[i].i);
			}
			...
			for (i=0;i<num_qubit;i++)
				{frame->literal[anticommute_row][i] = 0;}
			frame->literal[anticommute_row][qubit_index] = 1;
			//Step 3: Swap variables accordingly to align same phase vector sequence in both list 1 and list2
			for (i=0;i<total_state;i++)
			{
				if (swap_flag[i] == 0)
				{
					//Identify corresonding phase vector set that has same phase sign except for qubit_index position
					swap_index1 = i;
					found = 0;
					for (j=i+1;j<total_state;j++)
					{
						for (l=0;l<num_qubit;l++)
						{
							if((frame->phase[j][l] == frame->phase[swap_index1][l]) && (l != anticommute_row) || (frame->phase[j][l] != frame->phase[swap_index1][l]) && (l == anticommute_row))
								found = 1;
							else
							{found = 0; break;}  
						}
					...
					//Swap corresponding amp_ref and basis
					unsigned_temp = basis_index2[swap_index2];
					basis_index2[swap_index2] = basis_index2[swap_index1];
					basis_index2[swap_index1] = unsigned_temp;

					amplitude2[swap_index1].r = frame->amplitude[swap_index2].r;
					amplitude2[swap_index1].i = frame->amplitude[swap_index2].i; 
					amplitude2[swap_index2].r = frame->amplitude[swap_index1].r;
					amplitude2[swap_index2].i = frame->amplitude[swap_index1].i; 
					//Flag swapped pair
					swap_flag[swap_index1] = 1; swap_flag[swap_index2] = 1;
				}
			}
			
			//Perform canonical reduction to ensure correct basis amplitude extraction
			canonical_reduction (&*frame, num_qubit, total_state);
			//Update amplitude vector accordingly with correct factor and addition of same phase vector
			for (i=0;i<total_state;i++)
			{
				extract_specific (&*frame, i, num_qubit, basis_index[i], &amp_gen);
				complex_div(amp_ref[i], amp_gen, &factor);
				complex_mul(factor, frame->amplitude[i], &frame->amplitude[i]);
				extract_specific (&*frame, i, num_qubit, basis_index2[i], &amp_gen2);
				complex_div(amp_ref2[i], amp_gen2, &factor2);
				complex_mul(factor2, amplitude2[i], &amplitude2[i]);
				frame->amplitude[i].r += amplitude2[i].r;
				frame->amplitude[i].i += amplitude2[i].i;
			}
		}		
	}
...
{ 
	...
	printf("\t\t--QFT State Vector Approach: %u-qubit QFT--\n", num_qubit);
	...
	{
		//To discard redundant input such as next line from file
		do
	...
	/************************************APPLY RANDOM SEQUENCE STABILIZER GATES ON BASIS STATE************************************/
	//This is to create random stabilizer state as input to QFT
	//Store alpha in the amplitude slots
	//Allocate max number of memory for possible amplitude pairs
	...
	for (i=0;i<gate_sequence.size;i++)
	{
		if (gate_sequence.type[i]==0)		//Hadamard Gate
		{
			...
			get_alpha_H (&frame, num_qubit, gate_sequence.pos[2*i], basis_index);
			//Step 2: Conjugation-by-action
			conjugate_H (&frame, num_qubit, gate_sequence.pos[2*i]);			
		}
		else if (gate_sequence.type[i]==1)	//Phase Gate
		{
			//printf("\n\t\t--Gate Sequence %u: Apply Phase Gate on Qubit %u--\n", i, gate_sequence.pos[2*i]);
			//Step 1: Obtain alpha
			get_alpha_P (&frame, num_qubit, gate_sequence.pos[2*i], basis_index);
			//Step 2: Conjugation-by-action
			conjugate_P (&frame, num_qubit, gate_sequence.pos[2*i]);
		}
		else if (gate_sequence.type[i]==2)	//CNOT Gate
		{
			//printf("\n\t\t--Gate Sequence %u: Apply CNOT Gate on Target Qubit %u; Control Qubit %u--\n", i, gate_sequence.pos[2*i], gate_sequence.pos[2*i+1]);
			//Step 1: Obtain alpha
			get_alpha_CNOT (&frame, num_qubit, total_state, gate_sequence.pos[2*i], gate_sequence.pos[2*i+1], basis_index);
			//Step 2: Conjugation-by-action
			conjugate_CNOT (&frame, num_qubit, gate_sequence.pos[2*i], gate_sequence.pos[2*i+1]);
		}
	...
	//Output frame & Phase factor
	printf("\t--Intermediate Frame: After Random Stabilizer Gate Sequence--\n");
	print_frame(frame,num_qubit);
	basis_amplitude (frame, num_qubit, total_state);


	/**************************************************APPLY N-QUBIT QFT*********************************************************/
	unsigned int unitary=0; 	

	printf("\n\t\t~~Apply Gate Sequence Based On QFT Circuit~~\n");
	for (i=0;i<num_qubit;i++)
	{
		for (j=0;j<num_qubit-i;j++)
		{
			unitary++;
			if(j==0)
			{
				//Apply Hadamard gate
				printf("Unitary %u: Hadamard on qubit %u\n", unitary, i);
				//Step 1: Obtain alpha - for global phase maintenance
				get_alpha_H (&frame, num_qubit, i, basis_index);
				//Step 2: Conjugation-by-action
				conjugate_H (&frame, num_qubit, i);
				//Step 3: Canonical form reduction
				canonical_reduction (&frame, num_qubit, total_state);
				//Step 4: Obtain beta & update amplitude vector(s) - for global phase maintenance
				get_beta (&frame, num_qubit, basis_index);
			...
			else
			{
				...
				printf("Unitary %u: Controlled phase shift %u on control qubit %u & target qubit %u\n", unitary, j+1, i+j, i);
				apply_controlled_phase (&frame, num_qubit, total_state, i+j, i, j+1);	
			}
			printf("\t--Intermediate Frame--\n");
			print_frame(frame,num_qubit);
			//Extract basis states from stabilizer frame with multiple vector pairs
			basis_amplitude (frame, num_qubit, total_state);
		}
	}
		
	//Apply swap gate to swap the output sequence
	printf("Unitary %u: SWAP Output Qubits\n", unitary+1);
	apply_swap (&frame, num_qubit);
	//Ensure stabilizer frame in canonical reduction form
	canonical_reduction (&frame, num_qubit, total_state);
	printf("\t--Intermediate Frame--\n");
	print_frame(frame,num_qubit);
	//Extract basis states from stabilizer frame with multiple vector pairs
