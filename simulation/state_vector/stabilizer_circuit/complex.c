...
//	b->a = atan(a.i / a.r);
	float temp;
	temp = atan(a.i / a.r);
	if(temp<0)			//Make sure angle within the range [0,2*PI], not sure if correct!
		b->a = temp + (2*PI);
	else
		b->a = temp;
	return;
...
void polar_mul(COMPLEX_POLAR a, COMPLEX_POLAR b, COMPLEX_POLAR *c)	//Sometimes couldn't get same result as in Cartesian!
{
	c->m = a.m * b.m;
	//c->a = a.a + b.a;
	float temp;			//Make sure angle within the range [0,2*PI], not sure if correct!
	temp = a.a + b.a;
	if(temp > (2*PI))
		c->a = temp - (2*PI);
	else
...
void polar_div(COMPLEX_POLAR a, COMPLEX_POLAR b, COMPLEX_POLAR *c)	//Sometimes couldn't get same result as in Cartesian!
{
	c->m = a.m / b.m;
	//c->a = a.a - b.a;
	float temp;			//Make sure angle within the range [0,2*PI], not sure if correct!
